...
A fair question: should direct booking count now if the site is a year away? Rerun with its weight at 1. Tool X scores 97 / 27 = 3.59 and Tool Y scores 105 / 27 = 3.89. Tool Y still leads. There is also a cost the table does not show. Starting on Tool X and moving to Tool Y in a year means a second setup and a migration while the house is booked. Airbnb has a [help article on switching software](https://www.airbnb.com/help/article/2958) because this step has its own risks. Recommendation for Cedar Ridge: Tool Y, on condition that its all-in price is confirmed in SR.5.7 and Maya and Chris accept that it runs above the $100 budget if it does.
...
**Confirm with:** the state and local tax agencies in SR.1 Section D, which taxes you must remit on direct bookings and on any channel where you become merchant of record.
